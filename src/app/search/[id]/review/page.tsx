...
export default function ReviewPage({ params }: ReviewPageProps) {
		...
		const fetchListings = async () => {
			try {
				...
				const response = await fetch(
					`/api/configurations/${configurationId}/unrated`,
				);
	...
	const handleReset = useCallback(async () => {
		...
		if (
			!confirm(
				"Clear all passes and dislikes? You'll be able to review them again. (Your likes will be kept)",
			)
		) {
			return;
		}
		...
		try {
			setResetting(true);
			const response = await fetch(
				`/api/configurations/${configurationId}/reset`,
				{
					method: "DELETE",
				},
			);
