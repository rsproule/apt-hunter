...
export const runAllSearches = task({
  ...
  run: async () => {
    ...
    console.log(`📊 Found ${configurations.length} enabled pipelines to process`);
    ...
    for (const config of configurations) {
      try {
        ...
        while (Date.now() - startTime < MAX_WAIT_TIME) {
          ...
          console.log(`  ⏳ Enhancement status: ${enhancementStatus.status}, waiting...`);
          await wait.for({ seconds: POLL_INTERVAL / 1000 });
        }

        if (!enhancementComplete) {
          console.warn(`  ⚠️  Enhancement ${enhancement.id} did not complete in time - skipping notifications`);
          results.push({
        ...
        const topListings = await getTopUnsentListings(config.id, enhancement.id, 2);

        let sentCount = 0;
        for (const listing of topListings) {
          console.log(`  📬 Sending notification ${sentCount + 1} for listing to user: ${config.userId}`);
        ...
        console.error(`  ❌ Error processing configuration ${config.id}:`, error);
...
export const runSingleSearch = task({
  ...
  run: async (payload: { configurationId: string; userId: string }) => {
    ...
    console.log(`🚀 Starting single search for configuration: ${configurationId}`);
    ...
    let sentCount = 0;
    for (const listing of topListings) {
      console.log(`  📬 Sending notification ${sentCount + 1} for listing to user: ${userId}`);
